...
export const UPDATE_URL = 'https://raw.githubusercontent.com/BluntlyCat/TouchPortal-SCLogEventHandler/main/package.json';
export const RELEASE_URL = 'https://raw.githubusercontent.com/BluntlyCat/TouchPortal-SCLogEventHandler/releases';
export const SC_KILL_REGEX = /^<(?<timestamp>\d+-\d+-\d+T\d+:\d+:\d+\.\d+Z)>.+?Actor Death.+?Kill: '(?:(PU_\w+_(?<npc>NPC(?:_\w+)+))_\d+|(?<player>[A-Za-z0-9_-]+))'.+?in zone '(?<zone>[\w_-]+)'.+?killed by '(?<killer>\w+)'.+?with damage type '(?<dmgType>\w+)'/;